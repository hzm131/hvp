...
func (this *Reply) QueryReply(commentId string, limit string, offset string) (totalReply TotalReply, err error) {
	...
	count := servser_model.Db.Raw(`select reply.id,reply.content,reply.reply_id,reply.comment_id,reply.general_user_id,reply.awesome,reply.created_at,reply_user.id,reply_user.username,ar.id,ar.src,general_user.id,general_user.username,avatar.id,avatar.src
	...
	rows, err := servser_model.Db.Raw(`select reply.id,reply.content,reply.reply_id,reply.comment_id,reply.general_user_id,reply.awesome,reply.created_at,reply_user.id,reply_user.username,ar.id,ar.src,general_user.id,general_user.username,avatar.id,avatar.src
                     from comment inner join reply on
                             comment.id = reply.comment_id inner join general_user on
                             general_user.id = reply.general_user_id inner join general_user as reply_user on
                             reply_user.id = reply.reply_id inner join avatar on
